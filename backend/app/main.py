...
class ProcessInput(BaseModel): as_of:str; geography:str=Field(min_length=1,max_length=200); domain:str=Field(min_length=1,max_length=100); event_type:str=Field(min_length=1,max_length=100)
class OutcomeInput(BaseModel): prediction_id:str=Field(min_length=1,max_length=200); outcome_time:datetime; target:str=Field(min_length=1,max_length=200); observed:int=Field(ge=0,le=1); provenance:list[str]=Field(min_length=1,max_length=20)
...
async def ingest(request:Request,observations:list[ObservationInput]):
    _role(request,{"INGESTOR","ANALYST"}); rows=[Observation(**{**item.model_dump(),"event_time":datetime.fromisoformat(item.event_time),"publication_time":datetime.fromisoformat(item.publication_time),"acquisition_time":datetime.fromisoformat(item.acquisition_time),"provenance":tuple(item.provenance),"transformation_lineage":tuple(item.transformation_lineage)}) for item in observations]; return {"accepted":request.app.state.runtime.ingest(rows)}
@app.post("/v1/process")
async def process(request:Request,payload:ProcessInput):
    _role(request,{"ANALYST"}); result=request.app.state.runtime.process(as_of=datetime.fromisoformat(payload.as_of),geography=payload.geography,domain=payload.domain,event_type=payload.event_type); return {"event_id":result.event_id,"signal_ids":result.signal_ids,"pattern_id":result.pattern_id,"trajectory_id":result.trajectory_id,"alert":{"alert_id":str(result.alert.alert_id),"level":result.alert.level,"score":result.alert.score,"uncertainty":result.alert.uncertainty,"provenance":result.alert.provenance}}
