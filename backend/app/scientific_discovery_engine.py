...
class ScientificWork:
    ...
    def __post_init__(self)->None:
        ...
        if not self.provenance or not self.capability_not_authorized: raise ValueError("scientific work requires provenance and capability boundary")
...
def admissible_work(work:ScientificWork)->bool:
    return bool(work.provenance and work.alternative_explanations and work.falsification and work.benchmark and work.validation and work.stopping_rule.strip() and work.capability_not_authorized)
...
def adversarial_update(claim:ScientificClaim,*,interpretation_survived:bool|None,evidence_ids:tuple[str,...]=())->ScientificClaim:
    ...
    if interpretation_survived is None:
        return ScientificClaim(claim.claim_id,claim.statement,claim.state,evidence,claim.assumptions,claim.provenance,claim.capability_authorized,claim.capability_not_authorized)
