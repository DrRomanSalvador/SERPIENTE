...
class ExecutionOutcome(StrEnum):
    EXECUTED="EXECUTED"; INCONCLUSIVE="INCONCLUSIVE"; REJECTED="REJECTED"; BLOCKED_EXTERNAL="BLOCKED_EXTERNAL"
...
def _base_result(work:ScientificWork,obj:Observation|Signal|Forecast,outcome:ExecutionOutcome,finding:str,evidence:tuple[str,...],state:str,new_work:bool)->ScientificWorkResult:
    return ScientificWorkResult(work.work_id,_runtime_id(obj),outcome,finding,evidence,state,new_work,work.capability_not_authorized,tuple(dict.fromkeys((*work.provenance,f"runtime:{_runtime_id(obj)}"))))
...
def execute_scientific_work(work:ScientificWork,obj:Observation|Signal|Forecast)->ScientificWorkResult:
    ...
    if isinstance(obj,Signal):
        if obj.event_time is None:
            return _base_result(work,obj,ExecutionOutcome.INCONCLUSIVE,"signal is numerically well-formed and provenance-bearing, but event_time is absent; temporal precedence, lead time, and change-point claims are not identifiable from this object",(f"anomaly_score={obj.anomaly_score}",f"trend={obj.trend}",f"acceleration={obj.acceleration}",f"volatility={obj.volatility}"),"SIGNAL",False)
