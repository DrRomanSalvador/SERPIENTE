...
CREATE TYPE epistemic_state AS ENUM (
    'FACT_DOCUMENTED', 'DECLARATION', 'EVIDENCE_SUPPORTED', 'EVIDENCE_CONTRADICTED',
    'INFERENCE', 'HYPOTHESIS', 'ALTERNATIVE_HYPOTHESIS', 'UNCERTAIN', 'DISPROVEN', 'RETRACTED'
);
...
CREATE TYPE analysis_domain AS ENUM (
    'BIOLOGICAL_HEALTH', 'ENVIRONMENTAL_CLIMATE', 'VIOLENCE_ESCALATION',
    'SOCIOLOGICAL_POLARIZATION', 'DISINFORMATION', 'INFLUENCE_PROPAGANDA',
    'BORDER_GEOPOLITICAL', 'TERRITORIAL_INTEGRATED', 'INFRASTRUCTURE_SERVICES', 'ECONOMIC_SOCIAL'
);

-- Tipo de fuente
CREATE TYPE source_type AS ENUM (
    'GOVERNMENT_OFFICIAL', 'GOVERNMENT_AGENCY', 'INTERNATIONAL_ORG', 'ACADEMIC_RESEARCH',
    'NEWS_MEDIA', 'SOCIAL_MEDIA', 'NGO', 'THINK_TANK', 'CORPORATE', 'CITIZEN_REPORT',
    'SENSOR_IOT', 'SATELLITE', 'HEALTH_SYSTEM', 'ENVIRONMENTAL_MONITOR', 'SECURITY_FORCE',
    'BORDER_CONTROL', 'FINANCIAL_DATA', 'TELECOM_DATA', 'OTHER'
);

-- Calidad de fuente
CREATE TYPE source_quality AS ENUM (
    'VERIFIED_HIGH', 'VERIFIED_MEDIUM', 'VERIFIED_LOW', 'UNVERIFIED', 'DISCREDITED', 'MALICIOUS'
);

-- Independencia de fuente
CREATE TYPE source_independence AS ENUM (
    'PRIMARY', 'SECONDARY_INDEPENDENT', 'SECONDARY_DEPENDENT', 'TERTIARY', 'UNKNOWN'
);

-- Tipo de documento
CREATE TYPE document_type AS ENUM (
    'OFFICIAL_REPORT', 'SCIENTIFIC_PAPER', 'NEWS_ARTICLE', 'SOCIAL_MEDIA_POST',
    'GOVERNMENT_DOCUMENT', 'LEGAL_DOCUMENT', 'HEALTH_RECORD', 'ENVIRONMENTAL_DATA',
    'ECONOMIC_DATA', 'SECURITY_REPORT', 'BORDER_REPORT', 'SENSOR_DATA', 'SATELLITE_IMAGE',
    'AUDIO_TRANSCRIPT', 'VIDEO_TRANSCRIPT', 'DATABASE_RECORD', 'API_RESPONSE', 'OTHER'
);

-- Tipo de observación
CREATE TYPE observation_type AS ENUM (
    'EVENT', 'STATE', 'TREND', 'ANOMALY', 'CORRELATION', 'CAUSATION', 'PREDICTION', 'RETRODICTION'
);

-- Tipo de evidencia
CREATE TYPE evidence_type AS ENUM (
    'DIRECT_OBSERVATION', 'DOCUMENTARY', 'STATISTICAL', 'EXPERT_TESTIMONY',
    'PHYSICAL', 'DIGITAL', 'CIRCUMSTANTIAL', 'ANALOGICAL'
);

-- Tipo de hipótesis
CREATE TYPE hypothesis_type AS ENUM (
    'EXPLANATORY', 'PREDICTIVE', 'CAUSAL', 'CORRELATIONAL', 'MECHANISTIC', 'STATISTICAL'
);
...
CREATE TYPE intervention_type AS ENUM (
    'INFORMATION_PUBLIC', 'EDUCATION', 'COMMUNITY_BUILDING', 'INSTITUTIONAL',
    'POLICY', 'TECHNICAL', 'EMERGENCY', 'PREVENTIVE'
);

-- Tipo de sesión emocional
CREATE TYPE session_type AS ENUM (
    'VOLUNTARY_REFLECTION', 'EMOTIONAL_CHECK', 'STRESS_ASSESSMENT', 'WELLBEING_TRACKING'
);

-- Tipo de consulta médica
CREATE TYPE medical_consultation_type AS ENUM (
    'GENERAL_INQUIRY', 'SYMPTOM_CHECK', 'PROFESSIONAL_CONSULTATION', 'FOLLOW_UP'
);

-- Estado de auditoría
CREATE TYPE audit_action AS ENUM (
    'CREATE', 'READ', 'UPDATE', 'DELETE', 'EXPORT', 'IMPORT', 'LOGIN', 'LOGOUT',
    'PERMISSION_CHANGE', 'SECURITY_EVENT', 'SYSTEM_EVENT', 'DATA_ACCESS', 'ALERT_TRIGGERED',
    'MODEL_INFERENCE', 'RAG_QUERY', 'EXTERNAL_API_CALL'
);
...
CREATE TABLE sources (
    ...
    domains analysis_domain[], tags TEXT[],
...
-- 2. Usuarios (bootstrap 000_users.sql; retained as idempotent schema declaration)
...
CREATE TABLE documents (
    ...
    content_type VARCHAR(100), content_size_bytes BIGINT, language VARCHAR(10),
    source_id UUID NOT NULL REFERENCES sources(id),
    original_url VARCHAR(2048), published_at TIMESTAMPTZ,
    retrieved_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    domains analysis_domain[],
    epistemic_state epistemic_state NOT NULL DEFAULT 'DECLARATION',
    confidence_score DECIMAL(5,4) CHECK (confidence_score >= 0 AND confidence_score <= 1),
    storage_provider VARCHAR(50), storage_bucket VARCHAR(255), storage_key VARCHAR(500), storage_url VARCHAR(2048),
    processing_status VARCHAR(50) NOT NULL DEFAULT 'PENDING', processed_at TIMESTAMPTZ, processing_error TEXT,
...
CREATE TABLE observations (
    ...
    type observation_type NOT NULL, domains analysis_domain[], entities JSONB, keywords TEXT[],
    observed_at TIMESTAMPTZ, extracted_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    confidence_score DECIMAL(5,4) CHECK (confidence_score >= 0 AND confidence_score <= 1),
...
CREATE TABLE evidences (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    title VARCHAR(500) NOT NULL, description TEXT, type evidence_type NOT NULL,
    ...
    independence_count INTEGER NOT NULL DEFAULT 1,
...
CREATE TABLE claims (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    text TEXT NOT NULL, text_hash VARCHAR(64) NOT NULL, title VARCHAR(500),
    epistemic_state epistemic_state NOT NULL DEFAULT 'DECLARATION',
    confidence_score DECIMAL(5,4) CHECK (confidence_score >= 0 AND confidence_score <= 1),
    uncertainty_score DECIMAL(5,4) CHECK (uncertainty_score >= 0 AND uncertainty_score <= 1),
    supporting_evidence_ids UUID[] REFERENCES evidences(id), contradicting_evidence_ids UUID[] REFERENCES evidences(id),
    source_ids UUID[] REFERENCES sources(id), document_ids UUID[] REFERENCES documents(id), domains analysis_domain[],
    first_seen_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), last_updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), trend VARCHAR(20),
...
CREATE TABLE hypotheses (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, description TEXT NOT NULL,
    type hypothesis_type NOT NULL, status VARCHAR(50) NOT NULL DEFAULT 'PROPOSED',
    confidence_score DECIMAL(5,4) CHECK (confidence_score >= 0 AND confidence_score <= 1),
    supporting_evidence_ids UUID[] REFERENCES evidences(id), contradicting_evidence_ids UUID[] REFERENCES evidences(id),
    explains_claim_ids UUID[] REFERENCES claims(id), domains analysis_domain[], prior_probability DECIMAL(5,4), posterior_probability DECIMAL(5,4),
...
-- 9. Hipótesis competidoras
CREATE TABLE competing_hypotheses (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), hypothesis_a_id UUID NOT NULL REFERENCES hypotheses(id) ON DELETE CASCADE,
    hypothesis_b_id UUID NOT NULL REFERENCES hypotheses(id) ON DELETE CASCADE, competition_type VARCHAR(50), notes TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), CONSTRAINT competing_hypotheses_unique UNIQUE (hypothesis_a_id, hypothesis_b_id)
);

-- SIGNALS & ANOMALIES
CREATE TABLE signals (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, description TEXT,
    type VARCHAR(100) NOT NULL, domains analysis_domain[], magnitude DECIMAL(10,4), velocity DECIMAL(10,4), acceleration DECIMAL(10,4),
    is_anomalous BOOLEAN NOT NULL DEFAULT false, anomaly_score DECIMAL(5,4), baseline_value DECIMAL(10,4),
    detected_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), valid_from TIMESTAMPTZ, valid_until TIMESTAMPTZ,
    observation_ids UUID[] REFERENCES observations(id), evidence_ids UUID[] REFERENCES evidences(id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
    CONSTRAINT signals_title_unique UNIQUE (title)
);
CREATE INDEX idx_signals_type ON signals(type); CREATE INDEX idx_signals_domains ON signals USING GIN(domains);
CREATE INDEX idx_signals_anomalous ON signals(is_anomalous); CREATE INDEX idx_signals_detected ON signals(detected_at); CREATE INDEX idx_signals_magnitude ON signals(magnitude);

CREATE TABLE anomalies (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), signal_id UUID NOT NULL REFERENCES signals(id), anomaly_type VARCHAR(100) NOT NULL,
    detection_method VARCHAR(100) NOT NULL, detection_score DECIMAL(5,4) NOT NULL, threshold DECIMAL(5,4), description TEXT NOT NULL,
    severity VARCHAR(20) NOT NULL, potential_impact TEXT, affected_domains analysis_domain[], status VARCHAR(50) NOT NULL DEFAULT 'DETECTED',
    confirmed_at TIMESTAMPTZ, confirmed_by UUID REFERENCES users(id), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
    CONSTRAINT anomalies_signal_unique UNIQUE (signal_id)
);
CREATE INDEX idx_anomalies_type ON anomalies(anomaly_type); CREATE INDEX idx_anomalies_severity ON anomalies(severity);
CREATE INDEX idx_anomalies_status ON anomalies(status); CREATE INDEX idx_anomalies_domains ON anomalies USING GIN(affected_domains);
...
CREATE TABLE risk_assessments (
    ...
    impact DECIMAL(5,4) NOT NULL CHECK (impact >= 0 AND impact <= 5), vulnerability DECIMAL(5,4) CHECK (vulnerability >= 0 AND vulnerability <= 1),
    risk_score DECIMAL(5,4) NOT NULL, conditions JSONB, triggers TEXT[], evidence_ids UUID[] REFERENCES evidences(id),
    signal_ids UUID[] REFERENCES signals(id), anomaly_ids UUID[] REFERENCES anomalies(id), hypothesis_ids UUID[] REFERENCES hypotheses(id),
    assessed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), valid_from TIMESTAMPTZ, valid_until TIMESTAMPTZ, review_at TIMESTAMPTZ,
    status VARCHAR(50) NOT NULL DEFAULT 'ACTIVE', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
    CONSTRAINT risk_assessments_title_unique UNIQUE (title)
);
CREATE INDEX idx_risk_type ON risk_assessments(type); CREATE INDEX idx_risk_domains ON risk_assessments USING GIN(domains);
CREATE INDEX idx_risk_score ON risk_assessments(risk_score); CREATE INDEX idx_risk_status ON risk_assessments(status);

CREATE TABLE risk_convergences (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, description TEXT,
    risk_assessment_ids UUID[] NOT NULL REFERENCES risk_assessments(id), domains analysis_domain[] NOT NULL,
    convergence_type VARCHAR(100), combined_risk_score DECIMAL(5,4) NOT NULL, amplification_factor DECIMAL(5,4),
    status VARCHAR(50) NOT NULL DEFAULT 'DETECTED', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
...
-- ALERTS
CREATE TABLE alerts (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, description TEXT NOT NULL, level alert_level NOT NULL,
    domains analysis_domain[] NOT NULL, risk_assessment_ids UUID[] REFERENCES risk_assessments(id), convergence_ids UUID[] REFERENCES risk_convergences(id),
    anomaly_ids UUID[] REFERENCES anomalies(id), signal_ids UUID[] REFERENCES signals(id), explanation TEXT, evidence_summary TEXT,
    recommended_actions TEXT[], intervention_types intervention_type[], triggered_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), expires_at TIMESTAMPTZ,
    resolved_at TIMESTAMPTZ, status VARCHAR(50) NOT NULL DEFAULT 'ACTIVE', notifications_sent BOOLEAN NOT NULL DEFAULT false,
    notification_channels TEXT[], created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
    acknowledged_at TIMESTAMPTZ, acknowledged_by UUID REFERENCES users(id), CONSTRAINT alerts_title_unique UNIQUE (title)
);
CREATE INDEX idx_alerts_level ON alerts(level); CREATE INDEX idx_alerts_domains ON alerts USING GIN(domains);
CREATE INDEX idx_alerts_status ON alerts(status); CREATE INDEX idx_alerts_triggered ON alerts(triggered_at);

-- SCENARIOS
CREATE TABLE scenarios (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, description TEXT NOT NULL, scenario_type VARCHAR(100), domains analysis_domain[],
    assumptions TEXT[] NOT NULL, probability DECIMAL(5,4) CHECK (probability >= 0 AND probability <= 1),
    confidence_score DECIMAL(5,4) CHECK (confidence_score >= 0 AND confidence_score <= 1), time_horizon VARCHAR(50), horizon_months INTEGER,
    valid_from TIMESTAMPTZ, valid_until TIMESTAMPTZ, risk_assessment_ids UUID[] REFERENCES risk_assessments(id),
    hypothesis_ids UUID[] REFERENCES hypotheses(id), signal_ids UUID[] REFERENCES signals(id), expected_impact TEXT, impact_severity risk_type,
    tracking_indicators TEXT[], status VARCHAR(50) NOT NULL DEFAULT 'ACTIVE', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id), CONSTRAINT scenarios_title_unique UNIQUE (title)
);
...
CREATE TABLE interventions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, description TEXT, type intervention_type NOT NULL, domains analysis_domain[],
    alert_ids UUID[] REFERENCES alerts(id), risk_assessment_ids UUID[] REFERENCES risk_assessments(id), actions TEXT[] NOT NULL,
    expected_outcome TEXT, success_metrics TEXT[], status VARCHAR(50) NOT NULL DEFAULT 'PROPOSED', proposed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    approved_at TIMESTAMPTZ, started_at TIMESTAMPTZ, completed_at TIMESTAMPTZ, assigned_to UUID REFERENCES users(id), actual_outcome TEXT,
    effectiveness_score DECIMAL(5,4), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    created_by UUID REFERENCES users(id), CONSTRAINT interventions_title_unique UNIQUE (title)
);
...
CREATE TABLE emotional_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), user_id UUID NOT NULL REFERENCES users(id), type session_type NOT NULL,
    content_hash VARCHAR(64), mood_score INTEGER CHECK (mood_score >= 1 AND mood_score <= 10), stress_level INTEGER CHECK (stress_level >= 1 AND stress_level <= 10),
    anxiety_level INTEGER CHECK (anxiety_level >= 1 AND anxiety_level <= 10), duration_minutes INTEGER, notes TEXT,
    is_completed BOOLEAN NOT NULL DEFAULT false, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), completed_at TIMESTAMPTZ,
...
CREATE TABLE medical_consultations (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), user_id UUID NOT NULL REFERENCES users(id), professional_id UUID REFERENCES users(id), type medical_consultation_type NOT NULL,
    content_hash VARCHAR(64), symptoms JSONB, diagnosis TEXT, diagnosis_code VARCHAR(50), recommendations TEXT[], prescriptions TEXT[], referrals TEXT[],
    status VARCHAR(50) NOT NULL DEFAULT 'OPEN', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), closed_at TIMESTAMPTZ, created_by UUID REFERENCES users(id),
    is_encrypted BOOLEAN NOT NULL DEFAULT true, is_hipaa_compliant BOOLEAN NOT NULL DEFAULT true,
    CONSTRAINT medical_consultations_user_check CHECK (user_id IS NOT NULL)
);
CREATE INDEX idx_medical_consultations_user ON medical_consultations(user_id); CREATE INDEX idx_medical_consultations_professional ON medical_consultations(professional_id);
CREATE INDEX idx_medical_consultations_status ON medical_consultations(status); CREATE INDEX idx_medical_consultations_created ON medical_consultations(created_at);

-- AUDIT
CREATE TABLE audit_log (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), action audit_action NOT NULL, entity_type VARCHAR(100), entity_id UUID, user_id UUID REFERENCES users(id),
    description TEXT, metadata JSONB, ip_address INET, user_agent TEXT, success BOOLEAN NOT NULL DEFAULT true, error_message TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX idx_audit_action ON audit_log(action); CREATE INDEX idx_audit_entity ON audit_log(entity_type, entity_id); CREATE INDEX idx_audit_user ON audit_log(user_id);
CREATE INDEX idx_audit_created ON audit_log(created_at); CREATE INDEX idx_audit_success ON audit_log(success);

-- AI/RAG
CREATE TABLE embeddings (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), document_id UUID REFERENCES documents(id) ON DELETE CASCADE,
    observation_id UUID REFERENCES observations(id) ON DELETE CASCADE, claim_id UUID REFERENCES claims(id) ON DELETE CASCADE,
    embedding vector(1536), model_name VARCHAR(100) NOT NULL, model_version VARCHAR(50), dimensions INTEGER NOT NULL,
    chunk_index INTEGER, chunk_start INTEGER, chunk_end INTEGER, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX idx_embeddings_document ON embeddings(document_id); CREATE INDEX idx_embeddings_observation ON embeddings(observation_id); CREATE INDEX idx_embeddings_claim ON embeddings(claim_id);
CREATE INDEX idx_embeddings_vector ON embeddings USING HNSW (embedding vector_cosine_ops);

CREATE TABLE rag_queries (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), query_text TEXT NOT NULL, query_hash VARCHAR(64), result_document_ids UUID[], result_observation_ids UUID[], result_claim_ids UUID[],
    result_count INTEGER, top_k INTEGER, similarity_threshold DECIMAL(5,4), user_id UUID REFERENCES users(id), session_id VARCHAR(100), response_text TEXT, response_model VARCHAR(100),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id)
);
...
CREATE TABLE system_config (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), key VARCHAR(255) NOT NULL UNIQUE, value JSONB NOT NULL, description TEXT, config_type VARCHAR(50), scope VARCHAR(50),
    domain analysis_domain, is_active BOOLEAN NOT NULL DEFAULT true, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
...
CREATE TABLE domain_config (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), domain analysis_domain NOT NULL UNIQUE, config JSONB NOT NULL, is_enabled BOOLEAN NOT NULL DEFAULT true,
    is_public BOOLEAN NOT NULL DEFAULT false, alert_thresholds JSONB, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id)
);
...
CREATE TABLE entity_relationships (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), entity_a_type VARCHAR(100) NOT NULL, entity_a_id UUID NOT NULL,
    entity_b_type VARCHAR(100) NOT NULL, entity_b_id UUID NOT NULL, relationship_type VARCHAR(100) NOT NULL,
    strength DECIMAL(5,4) CHECK (strength >= 0 AND strength <= 1), confidence DECIMAL(5,4) CHECK (confidence >= 0 AND confidence <= 1),
    description TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_by UUID REFERENCES users(id),
...
SELECT domain, COUNT(*) as total_documents,
